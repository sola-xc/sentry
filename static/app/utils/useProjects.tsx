...
function useProjects({
  ...
  // Existing projects from the store
  const [storeProjects, setStoreProjects] = useState<Project[]>([]);

  // listen for new projects from the store
  useEffect(() => {
    setStoreProjects(ProjectsStore.getAll());

    const unsubscribe = ProjectsStore.listen(
      () => setStoreProjects(ProjectsStore.getAll()),
      undefined
    );
  ...
  useEffect(() => {
    /**
     * This will fetch projects via API (using project slug) provided by the fetchQueue
     */
    ...
    loadAllProjects();
  }, [orgId, storeProjects, slugs, limit, allProjects, passthroughPlaceholderProject]);
